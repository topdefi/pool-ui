// config
var API = 'https://eldermine.com/api/'; // API address
var defaultPool = 'xnxi'; // Default Pool ID
var stratum = '127.0.0.1';
...
// Time convert Local -> EST
function convertLocalDateToESTDate(date, toEST) {
	date = new Date(date);
...
// Time convert EST -> Local
function convertESTDateToLocalDate(date) {
	var newDate = new Date(date.getTime()+date.getTimezoneOffset()*60*1000);
	var localOffset = date.getTimezoneOffset() / 60;
	var hours = date.getESTHours();
...
function loadStatsData() {
	...
	$.each(data.pools, function(index, value) {
		...
		$("#totalPaid").text(_formatter(value.totalPaid, 2,"") + " BTC");
...
var data = {
            ...
            Chartist.Line('#chartStatsNetworkHashRate', data, options, responsiveOptions);

///


/// networkDifficulty test
...
var data = {
            ...
            Chartist.Line('#chartStatsNetworkDifficulty', data, options, responsiveOptions);
    ...
    return $.ajax(API + 'pools/' + currentPool + '/miners')
        .done(function (data) {
            ...
            if (data.length > 0) {
                $.each(data, function (index, value) {
                    minerList += '<tr>';
                    minerList += '<td>' + value.miner.substring(0) + '</td>';
                    //minerList += '<td><a href="' + value.addressInfoLink + '" target="_blank">' + value.miner.substring(0, 12) + ' &hellip; ' + value.miner.substring(value.miner.length - 12) + '</td>';
                    //minerList += '<td><a href="https://blockscout.com/etc/mainnet/account" ' + value.minerAddressInfoLink + '" target="_blank">' + value.miner.substring(0) + ' &hellip; ' + value.miner.substring(value.miner.length) + '</td>';
...
function loadBlocksList() {
    return $.ajax(API + 'pools/' + currentPool + '/blocks?pageSize=100')
        .done(function (data) {
            ...
            if (data.length > 0) {
                $.each(data, function(index, value) {
		var createDate = convertLocalDateToESTDate(new Date(value.created),true);
...
function loadPaymentsList() {
	...
	if (data.length > 0) {
	$.each(data, function(index, value) {
	var createDate = convertLocalDateToESTDate(new Date(value.created),false);
		paymentList += '<tr>';
...
function loadConnectConfig() {
    return $.ajax(API + 'pools')
        .done(function (data) {
            ...
            $.each(data.pools, function (index, value) {
                if (currentPool === value.id) {
                    connectPoolConfig += '<tr><td>Coin Name</td><td>' + value.coin.name + " (" + value.coin.name + ")"  + '</td></tr>';
                    connectPoolConfig += '<tr><td>Payout Scheme</td><td>' + value.paymentProcessing.payoutScheme + '</td></tr>';
                    connectPoolConfig += '<tr><td>Algorithm</td><td>SHA256d</td></tr>';
                    connectPoolConfig += '<tr><td>Pool Address</td><td><a href="' + value.addressInfoLink + '" target="_blank">' + value.address.substring(0, 6) + ' &hellip; ' + value.address.substring(value.address.length - 8) + '</a></td></tr>';
                    //connectPoolConfig += '<tr><td>Pool Address</td><td><a href="' + value.addressInfoLink + '" target="_blank">' + value.address.substring(0, 67) + '</a></td></tr>';
                    ...
                    $.each(value.ports, function (port, options) {
                        //connectPoolConfig += '<tr><td>Port ' + port + ' </td><td>';
                        connectPoolConfig += '<tr><td>'+ "[" + options.name + "]  " + 'stratum+tcp://' + stratum +':' + port + '&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;' + ' </td><td>';
